-- src/plan.t
-- Bound → Plan phase implementation.
...
function Bound.Component:plan_impl(ctx)
...
-- Schema boundary: no params needed
function M.boundary(self)
    local ctx = PlanCtx.new()
    return self:plan_impl(ctx)
end

function M.plan_component(bound_component)
    local ctx = PlanCtx.new()
    return bound_component:plan_impl(ctx)
end
