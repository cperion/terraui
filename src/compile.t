-- src/compile.t
-- Plan → Kernel phase implementation.
-- Factory: receives types, installs internal ASDL methods, returns module.
--
-- The compile product is { fn = run_fn, state_t = frame_t }.
-- frame_t owns all per-frame runtime state: params, nodes, commands.
-- run_fn executes layout + hit_test + input + emit on &frame_t.

return terralib.memoize(function(types)

local Decl = types.Decl
local Plan = types.Plan
...
local struct Color { r: float; g: float; b: float; a: float }
local struct Vec2  { x: float; y: float }

local struct NodeState {
...
local struct ClipState   { count: int32 }
local struct ScrollState { _pad: uint8 }
local struct StubCmd     { seq: int32 }

local struct RectCmd {
    x: float; y: float; w: float; h: float
    color: Color
    opacity: float
    z: float
...
local terra approx_text_line_width(chars: int32, font_size: float, letter_spacing: float) : float
...
local terra approx_text_max_explicit_line_width(text: rawstring, font_size: float, letter_spacing: float) : float
...
local terra approx_text_explicit_line_count(text: rawstring) : int32
...
local terra approx_text_chars_per_line(max_width: float, font_size: float, letter_spacing: float) : int32
...
local terra place_wrapped_word(line_chars: int32, word_chars: int32, pending_space: int32, limit: int32, lines: &int32) : int32
...
local terra approx_text_wrapped_line_count(text: rawstring, max_width: float, font_size: float, letter_spacing: float) : int32
    if text == nil then return 1 end
...
function Plan.Component:compile_impl(ctx)
...
-- Schema boundary: text_backend is explicit semantic param
function M.boundary(self, text_backend)
    local ctx = CompileCtx.new(self, { text_backend = text_backend })
    return self:compile_impl(ctx)
end

function M.compile_component(plan_component, opts)
    opts = opts or {}
    local ctx = CompileCtx.new(plan_component, { text_backend = opts.text_backend })
    return plan_component:compile_impl(ctx)
end
